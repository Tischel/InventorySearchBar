...
namespace InventorySearchBar.Inventories
{
    internal class LargeInventory : CharacterInventory
    {
        ...
        {
            if (_node->UldManager.NodeListCount < 69) { return; }

            if (!Plugin.Settings.HightlightTabs && !forced) { return; }

            AtkResNode* firstBagTab = _node->UldManager.NodeList[69];
            ...
            AtkResNode* secondBagTab = _node->UldManager.NodeList[68];
        ...
        {
            if (_node->UldManager.NodeListCount < 69) { return -1; }
